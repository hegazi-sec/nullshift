...
(async () => {
  ...
  await section('10. Admin / Providers tab', async () => {
    ...
    ok('≥13 options in dropdown (auto + providers)', opts.length >= 13, true);
    const optVals = await Promise.all(opts.map(o => o.getAttribute('value')));
    for (const name of ['auto', 'anthropic', 'openai', 'gemini', 'groq', 'mistral', 'xai', 'cohere', 'together', 'perplexity', 'openrouter', 'deepseek', 'ollama']) {
    ...
    ok('≥12 provider cards', cards.length >= 12, true);

    const listText = (await page.locator('#providerList').textContent()).toLowerCase();
    for (const name of ['anthropic', 'openai', 'gemini', 'groq', 'mistral', 'grok', 'cohere', 'together', 'perplexity', 'openrouter', 'deepseek', 'ollama']) {
