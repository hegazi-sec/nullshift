...
used. Providers that are LLM-only (Claude, Groq, Mistral, etc.) are skipped
silently. If none qualify, RAG is disabled and chat still works without it.

Providers with embedding support: openai, gemini, cohere, ollama.
"""
...
def _try_provider_embedding(
    ...
    mistral, xai, together, perplexity, openrouter, deepseek) return None
    ...
    if provider_name == "cohere":
        key = eff("cohere_api_key", settings.COHERE_API_KEY)
        if not key:
            return None
        try:
            from chromadb.utils.embedding_functions import CohereEmbeddingFunction
            return CohereEmbeddingFunction(api_key=key, model_name=custom_model or "embed-english-v3.0")
        except Exception as e:
            logger.warning("Cohere embedding init failed: %s", e)
            return None
    ...
    # groq, mistral, xai, together, perplexity, openrouter, deepseek.
    return None
...
def _build_embedding_fn() -> Tuple[Optional[Any], Optional[str]]:
    ...
    Respects two settings written by the admin UI:
      ...
      rag_embedding_provider — 'auto' walks the provider chain; any specific
                               name (openai/gemini/cohere/ollama) goes direct.
    """
    ss = None
    try:
    ...
    logger.warning(
        ...
        "or add OpenAI / Gemini / Cohere / Ollama to your chain.",
        chain,
    )
    return None, None
